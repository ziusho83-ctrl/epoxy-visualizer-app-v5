...
function App() {
  ...
  const [companyName] = useState('Claude Flooring')
  const [companyContact] = useState('Phone • Email • Website')
  ...
  async function submitQuoteRequest() {
    ...
    try {
      ...
      const payload = {
        ...
        projectName: projectName.trim() || 'Claude Flooring Proposal',
        finishLabel,
        selectedSolid,
        selectedFlake,
        compare,
  ...
  async function saveDataUrlWithFallback(dataUrl: string, filename: string) {
    ...
    try {
      ...
      if (navigator.share && navigator.canShare?.({ files: [file] })) {
        await navigator.share({ files: [file], title: 'Claude Flooring Preview' })
        return
      }
    } catch {
  ...
  async function exportQuoteCard() {
    try {
      ...
      const label = projectName.trim() || 'Claude Flooring Proposal'
      ...
      const filename = `claude-flooring-quote-${Date.now()}.png`
  ...
  async function exportPreviewImage(kind: 'comparison' | 'after') {
    try {
      ...
      const filename = `claude-flooring-${kind}-${Date.now()}.png`
      const dataUrl = canvas.toDataURL('image/png')
      await saveDataUrlWithFallback(dataUrl, filename)
    } catch {
    ...
    <main className="wrap">
      <header className="brand-header">
        <div className="brand-copy">
          <p className="brand-kicker">Claude Flooring</p>
          <h1>Claude Flooring</h1>
          <p>Beautiful floors, visualized instantly.</p>
          <p className="brand-powered">App powered by AI.</p>
        </div>
        <img src="/luxshield-logo.png" alt="Claude Flooring logo" className="brand-logo" />
      </header>
...
Clear Floor Mask
      ...
      <section className="card configurator-card">
        <div className="section-title-row">
          ...
          <img src="/luxshield-logo.png" alt="Claude Flooring mark" className="section-logo" />
        </div>
